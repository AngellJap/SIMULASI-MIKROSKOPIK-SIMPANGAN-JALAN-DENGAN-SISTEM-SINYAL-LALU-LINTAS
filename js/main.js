...
function init() {
    ...
    if (radiusSlider) {
        radiusSlider.addEventListener("input", function() {
            ...
            drawLayout();
            // inform controller (if available)
    ...
    function animate(timestamp) {
        ...
        // draw layout on main canvas (this also updates laneCoordinates)
        drawLayout();

        // after layout redraw, keep controller informed (in case lanes changed interactively)
        if (typeof vehController.setLaneCoordinates === 'function') {
            vehController.setLaneCoordinates(laneCoordinates);
        }
        ...
        // DRAW DEBUG: controller-provided debug visuals (paths & points)
        // draw on vehicle canvas so align with vehicle coords
